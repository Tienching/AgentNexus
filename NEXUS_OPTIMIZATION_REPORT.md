...
> 对比分析：外部 nexus (`/home/ubuntu/Projects/nexus/nexus/`) vs 内嵌 nexus (`/home/ubuntu/Projects/agent-nexus_feature-dev/src/nexus/`)
> 生成时间：2026-04-08
...
| `agent/loop.py` | 仅 import 路径差异 | `nexus.` vs `src.nexus.`，逻辑完全一致 |
...
**现状**：内嵌 nexus 缺少 `heartbeat/` 模块，无法定期唤醒代理执行周期性任务。agent-nexus 中的 evolve 定时任务目前依赖 cron 系统，没有心跳决策机制。

**方案**：
1. 将外部 nexus 的 `heartbeat/service.py` 适配到 `src/nexus/heartbeat/`
2. 修改 import 路径：`nexus.` → `src.nexus.`
3. 在 agent-nexus 的主入口中集成 HeartbeatService，用于 evolve 定时任务的决策
4. HeartbeatService 可替代/增强现有的 cron 触发逻辑，提供更智能的"是否有事要做"决策

**实现文件**：
- 新建 `src/nexus/heartbeat/__init__.py`
- 新建 `src/nexus/heartbeat/service.py`（从外部版本适配）

---

#### 优化2：引入 Channels 渠道系统

**现状**：内嵌 nexus 缺少整个 `channels/` 模块。agent-nexus 目前通过自定义的方式（AG-UI 等）与用户交互，无法直接支持 Telegram/Discord/飞书等渠道。
...
**实现文件**：
- 新建 `src/nexus/channels/__init__.py`
- 新建 `src/nexus/channels/base.py`
- 新建 `src/nexus/channels/manager.py`
- 新建 `src/nexus/channels/registry.py`
- 按需新建具体渠道文件
...
**现状**：外部 nexus 有 `templates/` 目录，含 SOUL.md、TOOLS.md、HEARTBEAT.md、AGENTS.md、USER.md 等模板，并通过 `sync_workspace_templates()` 在启动时自动同步到工作区。内嵌版本缺少这些模板。

**方案**：
1. 将外部 nexus 的 `templates/` 目录适配嵌入
2. 将模板文件中的 "nexus" 品牌替换为 "agent-nexus"（或可配置）
3. 在 agent-nexus 启动时调用 `sync_workspace_templates()` 确保工作区文件完整
4. 添加 HEARTBEAT.md 模板支持心跳功能（配合优化1）

**实现文件**：
- 新建 `src/nexus/templates/__init__.py`
- 新建 `src/nexus/templates/SOUL.md`
- 新建 `src/nexus/templates/TOOLS.md`
- 新建 `src/nexus/templates/HEARTBEAT.md`
- 新建 `src/nexus/templates/AGENTS.md`
- 新建 `src/nexus/templates/USER.md`

---

#### 优化4：内嵌版本的 ContextBuilder 支持 Identity 定制

**现状**：`context.py` 中的 `_get_identity()` 方法硬编码返回 "nexus 🐈" 品牌标识。agent-nexus 作为上层项目，应能定制身份信息。
...
**实现文件**：
- 修改 `src/nexus/agent/subagent.py`

---

#### 优化6：ChannelManager 的消息重试机制可复用

**现状**：外部 nexus 的 `ChannelManager` 实现了 `_send_with_retry()` 方法，使用指数退避重试发送消息。agent-nexus 如果需要消息发送可靠性，可以复用这一模式。
...
**实现文件**：
- 新建 `src/nexus/utils/retry.py`

---

#### 优化7：Sync Workspace Templates 工具函数

**现状**：外部 nexus 的 `utils/helpers.py` 中包含 `sync_workspace_templates()` 函数，用于在启动时将模板文件同步到工作区。内嵌版本的 `helpers.py` 需确认是否包含此函数。

**方案**：
1. 确认内嵌版本是否有 `sync_workspace_templates`
2. 如缺失，从外部版本适配过来
3. 修改模板源路径为 `src/nexus/templates/`
...
```python
# src/nexus/heartbeat/__init__.py
"""Heartbeat service for periodic agent wake-up."""
```

```python
# src/nexus/heartbeat/service.py
# 从外部 nexus/nexus/heartbeat/service.py 适配
# 修改所有 import: nexus. → src.nexus.
# 核心逻辑不变：两阶段决策（skip/run）+ 定时触发
```

集成点：在 agent-nexus 主入口中：
```python
from src.nexus.heartbeat.service import HeartbeatService
...
```python
# src/nexus/agent/subagent.py 修改 _run_subagent 方法
# 将串行：
#   for tool_call in response.tool_calls:
#       result = await tools.execute(tool_call.name, tool_call.arguments)
# 改为并发：
...
1. **内嵌版本与外部版本核心逻辑高度一致**：agent/、bus/、command/、config/、cron/、mission/、providers/、security/、session/、skills/、utils/ 的代码逻辑完全相同，仅 import 路径不同（`nexus.` vs `src.nexus.`）。

2. **内嵌版本独有 evolve 系统**：这是 agent-nexus 的自进化引擎，外部 nexus 没有对应功能。`config/schema.py` 中额外定义了 `EvolutionConfig`。

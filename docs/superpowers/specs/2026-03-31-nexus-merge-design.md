# Nexus 源码合入 Agent-Nexus 设计文档

**Date**: 2026-03-31
**Status**: Approved
**Scope**: 将 nexus 引擎源码合入 agent-nexus，使其成为默认 Chat Provider

---

## 1. 目标

将 nexus 的 AgentLoop 引擎源码直接合入 agent-nexus 仓库（不作为外部依赖），成为默认的聊天引擎。用户发消息时不再启动 claude CLI 子进程，而是进程内调用 AgentLoop。orchestrator/mission skill 自动加载，用户通过自然语言即可安排任务。

## 2. 代码布局

### 2.1 复制范围

从 `~/Projects/nexus/nexus/` 复制到 `agent-nexus/src/nexus/`。
...
- `command/` — nexus 内置 slash commands (/help, /model, /stop, etc)
- `cron/` — Cron service
...
所有复制进来的文件中 `from nexus.xxx` → `from src.nexus.xxx`，`import nexus.xxx` → `import src.nexus.xxx`。
...
# 也替换字符串中的引用 (如 "nexus.xxx" 模块路径)
find src/nexus -name "*.py" -exec sed -i 's/"nexus\./"src.nexus./g' {} +
```

替换后用 `grep -rn "from nexus\." src/nexus/` 验证无遗漏。
...
│   │   ├── session/                 # Session
│   │   ├── command/                 # nexus slash commands
│   │   ├── cron/                    # Cron
│   │   ├── skills/                  # Built-in skills
│   │   └── utils/                   # Helpers
│   ├── providers/
│   │   ├── nexus/                 ← NexusExecutor (桥接层)
│   │   │   ├── executor.py
│   │   │   ├── adapter.py
│   │   │   ├── event_schema.py
│   │   │   └── session_bridge.py
...
├── prompts/
│   └── skills/
│       ├── orchestrator/            ← 注入到 nexus 的技能
│       └── mission/                 ← 注入到 nexus 的技能
└── tests/
    └── providers/
        └── nexus/                 ← 测试
```
...
NexusExecutor 不使用子进程，`_build_command()` 返回空列表。这是合法的——该方法只在 CLI 子进程 executor 中使用。

## 4. Skill 注入机制

### 4.1 SkillsLoader 增加 extra_skills_dirs

修改 `src/nexus/agent/skills.py`，`SkillsLoader.__init__` 增加 `extra_skills_dirs` 参数：
...
### 4.2 NexusExecutor 注入路径

在 `_NexusPool._create_loop()` 中，AgentLoop 创建后注入 nexus skills 路径：
...
- `prompts/skills/orchestrator/SKILL.md` 出现在 nexus 技能列表
- `prompts/skills/mission/SKILL.md` 出现在 nexus 技能列表
- Agent 可以自主 `read_file` 加载完整技能内容
- 用户说"帮我创建一个任务分析代码" → nexus 读取 orchestrator skill → 执行 `python3 prompts/skills/orchestrator/scripts/orchestrator.py create ...`
...
nexus executor 只处理普通聊天消息。

## 6. 依赖合并

将 nexus 核心运行时依赖合入 `pyproject.toml`：

| 依赖 | 用途 | 是否已有 |
|------|------|---------|
| `openai>=2.8.0` | LLM 调用 | 需新增 |
| `tiktoken>=0.12.0` | Token 计数 | 需新增 |
| `loguru>=0.7.3` | nexus 日志 | 需新增 |
...
StreamOrchestrator.stream_agui()
    ...
    │
    ▼
NexusExecutor.execute()
    │ 从 RequestModel 提取 content, session_id, cwd
    │ _NexusPool.get_or_create(workspace) → AgentLoop 单例
...
1. 复制 nexus 核心目录到 `src/nexus/`
2. 批量替换 import 路径
3. 验证 `python3 -c "from src.nexus.agent.loop import AgentLoop"` 通过

### Phase 2: NexusExecutor 签名修复
1. 修改 `execute()` 为 3 参数签名
2. 更新测试

### Phase 3: SkillsLoader 增加 extra_skills_dirs
1. 修改 `src/nexus/agent/skills.py`
2. 修改 `_NexusPool._create_loop()` 注入 nexus skills 路径

### Phase 4: 默认 Provider 切换
1. `_default_provider()` 改为 "nexus"
2. 更新受影响的测试
...
| 风险 | 概率 | 缓解 |
|------|------|------|
| Import 替换遗漏 | 中 | sed 替换后用 `python3 -c "import src.nexus"` 验证 |
| 循环依赖 | 低 | nexus 不依赖 agent-nexus 代码，单向依赖 |
| nexus 依赖冲突 | 低 | 版本约束已检查，无冲突 |
| 排除目录导致缺失 | 中 | 复制后立即跑 import 测试，缺什么补什么 |
| LLM API key 配置 | 低 | nexus 读 `~/.nexus/config.json` 或环境变量 |

...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="vhsdk",
        description="Virtual Human SDK CLI - 管理 channels 和 providers",
    )
    ...
    # install channel
    channel_parser = install_subparsers.add_parser("channel", help="安装 Channel")
    channel_parser.add_argument("name", help="Channel 名称 (wecom/slack/telegram)")
    ...
    list_parser.add_argument("--type", choices=["channel", "provider", "all"], default="all")
...
def cmd_install_channel(args, installer: PluginInstaller) -> int:
    """安装 Channel"""
    print(f"📦 安装 Channel: {args.name}")
    
    success = installer.install_channel(args.name)
    if success:
        print(f"✅ Channel '{args.name}' 安装成功！")
        print(f"\n配置文件已生成: {installer.get_config_path('channel', args.name)}")
        print("\n请编辑配置文件填写必要参数后启用。")
        return 0
    else:
        print(f"❌ Channel '{args.name}' 不存在或安装失败。")
        print(f"\n可用 Channels: {', '.join(installer.list_available_channels())}")
        return 1
...
def cmd_list(args, installer: PluginInstaller) -> int:
    ...
    if args.type in ("channel", "all"):
        print("Channels:")
        channels = installer.list_installed_channels()
        if channels:
            for ch in channels:
                status = "✅ 启用" if installer.is_enabled("channel", ch) else "⏸️  禁用"
                print(f"  - {ch} [{status}]")
        else:
            print("  (无)")
        print()
...
def cmd_config_show(args, installer: PluginInstaller) -> int:
    ...
    # 列出配置文件
    for sub in ["channels", "providers"]:
        sub_dir = installer.config_dir / sub
        if sub_dir.exists():
            print(f"{sub.capitalize()}:")
...
def main(argv=None) -> int:
    ...
    if args.command == "install":
        if args.type == "channel":
            return cmd_install_channel(args, installer)
        elif args.type == "provider":
            return cmd_install_provider(args, installer)
        else:
            print("请指定安装类型: vhsdk install channel <name> 或 vhsdk install provider <name>")
            return 1

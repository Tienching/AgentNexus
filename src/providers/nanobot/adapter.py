# -*- coding: utf-8 -*-
"""NanobotAGUIAdapter — converts internal NanobotEvents to AG-UI SSE.

This adapter sits between the raw JSON lines emitted by
:class:`NanobotExecutor` and the client expecting AG-UI protocol events.
"""
...
class NanobotAGUIAdapter(BaseAdapter):
    """AG-UI adapter for the Nanobot provider.
    ...
    def convert(self, event: Dict[str, Any]) -> Optional[str]:
        """Convert a single NanobotEvent dict to one or more AG-UI SSE events."""
        if not self.state:
            return None

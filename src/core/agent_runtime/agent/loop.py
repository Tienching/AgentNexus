...
from src.core.agent_runtime.agent.context import ContextBudget, ContextBudgetManager, ContextBuilder
from src.core.agent_runtime.agent.permissions import PermissionGate, PermissionMode, create_permission_gate
from src.core.agent_runtime.agent.hooks import ToolHooks
from src.core.agent_runtime.agent.memory import MemoryConsolidator
from src.core.agent_runtime.agent.subagent import SubagentManager
from src.core.agent_runtime.agent.tools.cron import CronTool
from src.core.agent_runtime.agent.skills import BUILTIN_SKILLS_DIR
from src.core.agent_runtime.agent.tools.filesystem import EditFileTool, ListDirTool, ReadFileTool, WriteFileTool
from src.core.agent_runtime.agent.tools.message import MessageTool
...
from src.core.agent_runtime.agent.tools.mission import MissionTool
from src.core.agent_runtime.agent.tools.spawn import SpawnTool
...
from src.core.agent_runtime.command import CommandContext, CommandRouter, register_builtin_commands
from src.core.agent_runtime.bus.queue import MessageBus
from src.core.agent_runtime.providers.base import LLMProvider
from src.core.agent_runtime.session.manager import Session, SessionManager

if TYPE_CHECKING:
    from src.core.agent_runtime.config.schema import ChannelsConfig, ExecToolConfig, WebSearchConfig
    from src.core.agent_runtime.cron.service import CronService
    from src.core.agent_runtime.swarm.permission_sync import PermissionSyncService
...
class AgentLoop:
    ...
    def __init__(
        ...
        # Mission service for long-running autonomous tasks
    ...
    async def _check_sync_permission(
        ...
        """
        from src.core.agent_runtime.swarm.permission_sync import PermissionSyncService
        from src.core.agent_runtime.agent.permissions import classify_tool
    ...
    def _register_default_tools(self) -> None:
        ...
        self.tools.register(MissionTool(service=self.mission_service))
    ...
    async def _connect_mcp(self) -> None:
        ...
        from src.core.agent_runtime.agent.tools.mcp import connect_mcp_servers
        try:
    ...
    def _set_tool_context(self, channel: str, chat_id: str, message_id: str | None = None) -> None:
        ...
        for name in ("message", "spawn", "cron", "mission"):
    ...
    def _strip_think(text: str | None) -> str | None:
        ...
        if not text:
            return None
        from src.core.agent_runtime.utils.helpers import strip_think
        ...
        *on_stream_end(resuming)*: called when a streaming session finishes.
        ...
        while iteration < self.max_iterations:
            ...
            if response.has_tool_calls:
                ...
                # Notify on_tool_start before execution
                if on_tool_start:
                    for tc in response.tool_calls:
                        try:
                            await on_tool_start(tc.name, tc.id, tc.arguments if isinstance(tc.arguments, dict) else {})
                        except Exception:
                ...
                for tool_call, result in zip(response.tool_calls, results):
                    ...
                    # Notify on_tool_end after each tool execution
                    if on_tool_end:
                        try:
                            result_str = result if isinstance(result, str) else str(result)
                            await on_tool_end(tool_call.id, result_str)
                        except Exception:
                            logger.warning("on_tool_end callback failed for {}", tool_call.name, exc_info=True)
    ...
    async def run(self) -> None:
        ...
        # Resume any missions that were running when we last shut down
        try:
            await self.mission_service.resume_interrupted()
        except Exception as e:
            logger.warning("Failed to resume interrupted missions: {}", e)

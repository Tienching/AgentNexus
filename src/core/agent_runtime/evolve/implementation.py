...
from src.core.agent_runtime.evolve.prompts import (
...
if TYPE_CHECKING:
    from src.core.agent_runtime.evolve.models import EvolutionSession, EvolutionTask
...
def run_shell(engine: "EvolutionEngine", cmd: str, timeout: int = 30, stdin_data: str | None = None) -> tuple[int, str, str]:
    """Run a shell command, return (exit_code, stdout, stderr).

    A best-effort guard rejects commands that look like they were built from
    unquoted, potentially hostile input; callers MUST shlex.quote every
    interpolated value. When ``stdin_data`` is given the command is piped via
    stdin (use with ``git commit -F -`` so commit messages cannot escape
    shell quoting).
    """
    try:
...
def get_git_diff_files(engine: "EvolutionEngine", base_sha: str) -> list[str]:
    ...
    code, stdout, _ = engine._run_shell(f"git diff --name-only {shlex.quote(base_sha)}..HEAD")
    if code != 0:
        return []
...
def cleanup_stale_worktrees(engine: "EvolutionEngine") -> None:
    ...
    for wt_dir in base.iterdir():
        if wt_dir.is_dir() and wt_dir.name.startswith("session-"):
            ...
            engine._run_shell(f"git worktree remove {shlex.quote(str(wt_dir))} --force")
            branch = f"evolve/{wt_dir.name}"
            engine._run_shell(f"git branch -D {shlex.quote(branch)}")
...
def create_worktree(engine: "EvolutionEngine", branch_name: str, worktree_path: Path) -> bool:
    ...
    code, _, err = engine._run_shell(f"git worktree add {shlex.quote(str(worktree_path))} -b {shlex.quote(branch_name)}")
    if code != 0:
...
def remove_worktree(engine: "EvolutionEngine", branch_name: str, worktree_path: Path) -> None:
    ...
    engine._run_shell(f"git worktree remove {shlex.quote(str(worktree_path))} --force")
    engine._run_shell(f"git branch -D {shlex.quote(branch_name)}")
...
def get_worktree_commits(engine: "EvolutionEngine", branch_name: str, base_sha: str) -> list[str]:
    ...
    _, stdout, _ = engine._run_shell(f"git log --oneline {shlex.quote(base_sha)}..{shlex.quote(branch_name)}")
    return [line.strip() for line in stdout.splitlines() if line.strip()]
...
async def run_task_in_worktree(
    ...
    _, stdout, _ = engine._run_shell(
        f"git diff --name-only {shlex.quote(base_sha)}..{shlex.quote(branch_name)} -- " + " ".join(shlex.quote(f) for f in engine.config.protected_files)
    )
    if stdout.strip():
    ...
    _, diff_out, _ = engine._run_shell(f"git diff --name-only {shlex.quote(base_sha)}..{shlex.quote(branch_name)}")
...
async def try_merge(
    ...
    code, _, err = engine._run_shell(
        f"git merge --no-ff {shlex.quote(result.branch_name)} -F -",
        timeout=30,
        stdin_data=commit_msg,
    )
    if code == 0:
    ...
    code2, _, _ = engine._run_shell(
        f"git merge --no-ff -X theirs {shlex.quote(result.branch_name)} -F -",
        timeout=30,
        stdin_data=f"{commit_msg} [auto-resolved]",
    )
    if code2 == 0:
        result.merge_status = "merged"
...
async def codebuddy_resolve_conflict(
    ...
    engine._run_shell(f"git merge --no-ff --no-commit {shlex.quote(result.branch_name)}", timeout=30)
    ...
    if not conflicted:
        code, _, _ = engine._run_shell("git commit -F -", stdin_data=commit_msg)
        if code == 0:
            return True
        engine._run_shell("git merge --abort")
        return False
    ...
    _, grep_out, _ = engine._run_shell('grep -r "<<<<<<" ' + " ".join(shlex.quote(f) for f in conflicted) + ' 2>/dev/null || true')
...
async def run_implementation_serial(engine: "EvolutionEngine", session: "EvolutionSession") -> None:
    ...
    for index, task in enumerate(tasks[:max_tasks]):
        ...
        if not is_valid:
            ...
            engine._run_shell(f"git reset --hard {shlex.quote(pre_sha)}")
            task.status = "failed"

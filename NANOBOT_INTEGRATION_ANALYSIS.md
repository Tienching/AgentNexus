# Nanobot 作为 agent-nexus 默认 Chat Provider — 深度分析

## 📌 目标

用户发消息时，默认走 nanobot 的 AgentLoop（in-process），而非启动 claude CLI 子进程。
orchestrator skill 自动加载到 nanobot 上下文，用户通过自然语言即可安排 task。

---

## 🔍 当前问题清单（feature-nanobot 分支的 Gap）
...
```
但我们的 `NanobotExecutor.execute()` 签名是：
```python
...
**修复**: NanobotExecutor.execute() 必须改成 3 参数签名 `(request, exec_user, output_format)`，
...
当默认改成 nanobot 后，slash commands 仍然会走 claude。但如果用户没装 claude CLI，
这会失败。

**方案**:
- 保留现有逻辑——slash commands 继续走 claude executor
- 或：在 NanobotExecutor 中检测 slash command，委托给 CLIExecutor

### ❌ P1: Adapter 位置与导入路径

**当前**: `src/providers/nanobot/adapter.py` 是独立实现
**其他 provider**: adapter 在 `src/runtime/adapters/{provider}/`

**影响**: `create_adapter("nanobot")` 导入路径 `src.providers.nanobot.NanobotAGUIAdapter`
可以工作，但不符合项目惯例。

**决定**: 保持在 `src/providers/nanobot/adapter.py`（简单，不影响功能）
...
**更好方案**: 不用 symlink，改为修改 nanobot 的 `SkillsLoader` 初始化参数，
传入额外的 skills 目录。

### ⚠️ P2: config.py default_provider 改了但没用

**ProviderSettings.default_provider** 改成 `"nanobot"` 了，
...
### ⚠️ P2: nanobot 初始化失败时的降级

如果 `nanobot` 包没安装或配置有误，`_NanobotPool._create_loop()` 会抛 ImportError。
`create_all_executors()` 在 StreamHandler.__init__() 调用——会导致**整个服务启动失败**。

**修复**: `create_all_executors()` 中 nanobot 应该 try/except，失败时 log warning 而不是 crash。
...
def create_all_executors(*, config=None) -> dict:
    ...
    try:
        executors["nanobot"] = create_executor("nanobot", config=config)
    except Exception as e:
        logger.warning("Nanobot provider unavailable: %s", e)
    return executors
```
...
这需要在 nanobot 的 `SkillsLoader` 中添加 `extra_skills_dirs` 支持，
在 `list_skills()` 中增加一个搜索层。

或者更简单的方案：通过 nanobot 的 `AGENTS.md` bootstrap 文件注入 skill 描述。

### Fix 4: 默认 provider 正确生效

只需确保 `_default_provider()` 返回 `"nanobot"`：
...
加上 `.env` 文件配置 `AGENT_NEXUS_DEFAULT_PROVIDER=nanobot` 作为部署默认。
...
| 文件 | 改动类型 | 描述 |
|------|----------|------|
| `src/providers/nanobot/executor.py` | **重写 execute()** | 匹配 3 参数签名 |
| `src/providers/dispatcher.py` | 修改 | create_all_executors 加 try/except |
| `src/providers/dispatcher.py` | 修改 | _default_provider 改成 "nanobot" |
| `nanobot/agent/skills.py` | 修改 | SkillsLoader 支持 extra_skills_dirs |
| `src/providers/nanobot/executor.py` | 修改 | skill 注入改用 SkillsLoader 参数 |
| 测试文件 | 更新 | 适配新签名 |
...
1. **默认值用 "nanobot" 还是 "claude"?**
   - 建议: 代码默认 "nanobot"，通过 env var 回退到 claude
...
4. **初始化失败降级策略?**
   - 建议: create_all_executors try/except，服务启动不因 nanobot 缺失而失败
